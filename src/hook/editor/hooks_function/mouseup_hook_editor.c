...
void	move_scene_during_right_clic(bool up, struct s_hook_data *hdata)
{
	...
	cam->rotation = (t_rotation){cam->rotation.pitch + (float)(pos.w - pos.y)
		/ RT_HEIGHT * 360, cam->rotation.yaw + (float)(pos.x - pos.z) / RT_WIDTH
		* 360, cam->rotation.roll + 0};
}
...
void	move_scene_during_left_clic(bool up, struct s_hook_data *hdata)
{
	...
	rt_mouse_get_pos(hdata->screen, &pos.z, &pos.w);
	rt_mouse_move(hdata->screen, pos.x, pos.y);
	cam->coord_translation += (t_v4f){(float)(pos.z - pos.x) / 10, (float)(pos.w
			- pos.y) / 10, 0, 0};
}
