...
/*   Updated: 2024/09/04 20:07:49 by bonsthie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
enum e_rtss_lex_attr_type __rtss_find_type(char **str)
{
	...
	type = rtss_get_name((const char **)str);
	if (!type)
		return (0);
	i = 0;
	char **type_attr = (char *[])RTSS_TYPE_ATTR;
	while (!ft_strcmp(type, type_attr[i]))
		i++;
	free(type);
	*str = end_type + 1;
	return (i);
...
static struct s_rtss_lex_attr	*__rtss_create_attr(char *str)
{
	...
	//
	new->value = __rtss_find_value(str);
	return (new);
}
...
static struct s_rtss_lex_attr	*__rtss_parse_attr(char *str)
{
	...
	return (__rtss_split_to_attr(split_args));
}
