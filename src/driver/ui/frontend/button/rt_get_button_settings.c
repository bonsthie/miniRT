...
/*   Updated: 2024/09/17 15:31:37 by bonsthie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_button *button)
{
	...
	if (settings->width == 0)
		settings->width = size + settings->padding.left
			+ settings->padding.right;
	if (settings->height == 0)
		settings->height = font_pixel_height + settings->padding.top
...
struct s_rtss_button_settings	button_default_settings(t_button *button)
{
	...
	settings.padding.data = hentry_get_spec(htab, button_hentry,
			(t_keyw)PADDING);
	settings.border_radius.data = hentry_get_spec(htab, button_hentry,
			(t_keyw)BORDER_RADIUS);
	if (button->settings.style & TXT)
		...
		t_hentry *hentry, enum e_button_action action)
{
	...
	settings.border_radius.data = hentry_get_spec(htab, hentry,
			(t_keyw)BORDER_RADIUS);
	return (settings);
}
		...
		t_button *button)
{
	...
	if (!hentry)
		return ;
	printf("class is here\n");
	tmp = __button_class_to_settings(button->settings.htab, hentry,
			button->action);
	if (tmp.color && tmp.color != RT_BUTTON_COLOR)
