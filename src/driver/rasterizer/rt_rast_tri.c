...
/*   Updated: 2024/06/07 15:14:57 by babonnet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	unsigned int endy = by  + 4;
	unsigned int endx = bx  + 4;
...
t_bbox calcul_bbox_triangle(t_tri triangle)
{
	...
	bbox.min_x = (int)fmin(triangle.vertex1.position.x, fmin(triangle.vertex2.position.x, triangle.vertex3.position.x));
	bbox.min_y = (int)fmin(triangle.vertex1.position.y, fmin(triangle.vertex2.position.y, triangle.vertex3.position.y));
	bbox.max_x = (int)fmax(triangle.vertex1.position.x, fmax(triangle.vertex2.position.x, triangle.vertex3.position.x));
	bbox.max_y = (int)fmax(triangle.vertex1.position.y, fmax(triangle.vertex2.position.y, triangle.vertex3.position.y));
...
void rast_tri(t_tri triangle, t_img *img, unsigned int color) 
{
    ...
    for (int by = bbox.min_y; by <= bbox.max_y; by += 4) {
        for (int bx = bbox.min_x; bx <= bbox.max_x; bx += 4) {
            ...
            int w0 = edge_coef.dwdx0 * (bx - triangle.vertex2.position.x) + edge_coef.dwdy0 * (by - triangle.vertex2.position.y);
            int w1 = edge_coef.dwdx1 * (bx - triangle.vertex3.position.x) + edge_coef.dwdy1 * (by - triangle.vertex3.position.y);
            int w2 = edge_coef.dwdx2 * (bx - triangle.vertex1.position.x) + edge_coef.dwdy2 * (by - triangle.vertex1.position.y);

            // Build masks
            unsigned int mask0 = build_mask_linear(w0, edge_coef.dwdx0, edge_coef.dwdy0);
            unsigned int mask1 = build_mask_linear(w1, edge_coef.dwdx1, edge_coef.dwdy1);
            unsigned int mask2 = build_mask_linear(w2, edge_coef.dwdx2, edge_coef.dwdy2);
